...
class AppUser {
  ...
  factory AppUser.fromJson(Map<String, dynamic> j) => AppUser(
    ...
    interestsLastChanged: j['interests_last_changed'] != null
        ? DateTime.tryParse(j['interests_last_changed'].toString())
        : null,
  );
}
...
class Course {
  ...
  final String? feesHome;
  final String? location;
  final String? url;
  final String? ucasCode;
  final String? startDate;
  final String? category;
  final String? courseType; // maps to course_type in DB — 'University' or 'Apprenticeship'
  ...
  // FIXED: use course_type column from the DB, not the title text.
  // Previously this checked if the title *contained* the word "apprenticeship"
  // which caused courses like "Apprentice Dental Nurse" (course_type=University)
  // to be wrongly shown in the Apprenticeship tab.
  bool get isApprenticeship {
    if (courseType != null && courseType!.isNotEmpty) {
      return courseType!.toLowerCase() == 'apprenticeship';
    }
    // Fallback only for rows that predate the course_type column
    return title.toLowerCase().contains('bootcamp');
  }
  ...
  factory Course.fromJson(Map<String, dynamic> j) => Course(
    ...
    title: j['title'],
    duration: j['duration'],
    feesHome: j['fees_home'],
    location: j['location'],
    url: j['url'],
    ucasCode: j['ucas_code'],
    startDate: j['start_date'],
    category: j['category'],
    courseType: j['course_type'], // reads the actual DB column
    institution: j['institutions'] != null
        ? Institution.fromJson(j['institutions'])
        : null,
  );
}

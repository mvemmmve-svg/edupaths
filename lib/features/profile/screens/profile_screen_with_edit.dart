...
// Added: Edit name and year group from Account Details sheet
// Uses update_my_profile RPC to save to DB
...
class ProfileScreen extends ConsumerWidget {
    ...
    return Scaffold(
      ...
      body: SafeArea(child: SingleChildScrollView(
        ...
        child: Column(children: [
          ...
          if (isLoggedIn) ...[
            // Account Details — now opens editable sheet
  ...
  // ── Account Details — now shows edit button ──────────────────────────────
    ...
    showModalBottomSheet(
      ...
      if (uid != null) {
        ...
        if (context.mounted) ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
          content: Text('✅ Matches updated!'), backgroundColor: AppColors.success));
      }
    });
  }
    ...
    showModalBottomSheet(
      ...
      builder: (ctx) => Padding(
        ...
        child: Column(mainAxisSize: MainAxisSize.min,
          ...
          const Text('Contact Us', style: TextStyle(fontFamily: 'Nunito', fontSize: 20, fontWeight: FontWeight.w900)),
          const SizedBox(height: 8),
          ...
          Consumer(builder: (c, r, _) {
            ...
            if (isPrem) {
              return PrimaryBtn(label: '💬 Message Support', onPressed: () {
                Navigator.pop(ctx);
                context.push(Supabase.instance.client.auth.currentUser != null
                    ? '/support' : AppConstants.routeLogin);
              });
            }
            return GestureDetector(
              ...
              child: Container(width: double.infinity, padding: const EdgeInsets.all(14),
                ...
                child: const Row(children: [
                  ...
                  Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                    Text('Live Chat Support', style: TextStyle(fontFamily: 'Nunito', fontSize: 14, fontWeight: FontWeight.w900)),
                    Text('Premium perk — or email us below.', style: TextStyle(fontFamily: 'Nunito', fontSize: 11.5, color: AppColors.textMid)),
                  ])),
          ...
          EduCard(child: Column(children: [
            _ContactRow(emoji: '📧', label: 'General Support', value: 'support@edupaths.co.uk'),
            const Divider(height: 1),
            _ContactRow(emoji: '🐛', label: 'Report a Bug', value: 'bugs@edupaths.co.uk'),
            const Divider(height: 1),
            _ContactRow(emoji: '💬', label: 'General Enquiries', value: 'hello@edupaths.co.uk'),
            const Divider(height: 1),
            _ContactRow(emoji: '🏫', label: 'School Partnerships', value: 'schools@edupaths.co.uk'),
          ])),
          const SizedBox(height: 16),
        ])));
  }
  ...
  void _showSettings(BuildContext context, WidgetRef ref) {
    showModalBottomSheet(
      ...
      builder: (ctx) => Consumer(builder: (ctx, ref, _) => Padding(
        ...
        child: Column(mainAxisSize: MainAxisSize.min,
          crossAxisAlignment: CrossAxisAlignment.start, children: [
          const Text('Settings', style: TextStyle(fontFamily: 'Nunito', fontSize: 20, fontWeight: FontWeight.w900)),
          const SizedBox(height: 20),
          _SettingRow(emoji: '🔒', label: 'Privacy Policy',
            trailing: const Icon(Icons.chevron_right_rounded, color: AppColors.textLight), onTap: () {}),
  ...
  Widget build(BuildContext context) => Padding(
    ...
    child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
      SizedBox(width: 110, child: Text(label, style: const TextStyle(
        fontFamily: 'Nunito', fontSize: 13, color: AppColors.textMid, fontWeight: FontWeight.w600))),
...
class _SettingRow extends StatelessWidget {
  ...
  const _SettingRow({required this.emoji, required this.label, required this.trailing, this.onTap});
  @override
  Widget build(BuildContext context) => GestureDetector(
    ...
    child: Padding(padding: const EdgeInsets.symmetric(vertical: 12),
      child: Row(children: [
        ...
        Expanded(child: Text(label, style: const TextStyle(fontFamily: 'Nunito', fontSize: 14, fontWeight: FontWeight.w700))),
        trailing,
      ])));
}
...
class _Stat extends StatelessWidget {
  ...
  final Color bg, fg;
  const _Stat({required this.value, required this.label, required this.emoji, required this.bg, required this.fg});
  @override
  Widget build(BuildContext context) => Container(
    ...
    child: Column(children: [
      ...
      Text(value, style: TextStyle(fontFamily: 'Nunito', fontSize: 20, fontWeight: FontWeight.w900, color: fg)),
      Text(label, style: const TextStyle(fontFamily: 'Nunito', fontSize: 10, color: AppColors.textMid, fontWeight: FontWeight.w600)),
    ]));
}
...
class _MenuItem extends StatelessWidget {
  ...
  const _MenuItem({required this.emoji, required this.label, required this.onTap, this.highlight = false});
  @override
  Widget build(BuildContext context) => Padding(
    ...
    child: EduCard(onTap: onTap, padding: const EdgeInsets.all(16),
      child: Row(children: [
        ...
        Expanded(child: Text(label, style: TextStyle(fontFamily: 'Nunito', fontSize: 14,
          fontWeight: FontWeight.w700,
          color: highlight ? AppColors.primary : AppColors.textDark))),
        Icon(Icons.chevron_right_rounded, color: highlight ? AppColors.primary : AppColors.textLight),
      ])));
}

// ── Edit Interests Sheet (unchanged) ─────────────────────────────────────────
...
class _EditInterestsSheetState extends ConsumerState<_EditInterestsSheet> {
  ...
  Future<void> _loadCurrent() async {
    ...
    setState(() {
      _selectedInterestIds = Set<String>.from((interests as List).map((e) => e['interest_id'] as String));
      _selectedTraitIds = Set<String>.from((traits as List).map((e) => e['trait_id'] as String));
      _loading = false;
    });
  }
        ...
        child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
          ...
          Row(children: [
            TextButton(onPressed: () => setState(() {
              if (_tab == 0) _selectedInterestIds.clear(); else _selectedTraitIds.clear();
            }), child: const Text('Clear All', style: TextStyle(
              fontFamily: 'Nunito', fontSize: 13, fontWeight: FontWeight.w700, color: AppColors.error))),
            TextButton(onPressed: _saving ? null : _save,
              child: _saving
                  ? const SizedBox(width: 16, height: 16, child: CircularProgressIndicator(strokeWidth: 2))
                  : const Text('Save', style: TextStyle(fontFamily: 'Nunito', fontSize: 14,
  ...
  Widget _buildInterests(AsyncValue<List<Interest>> interestsAsync) {
    return interestsAsync.when(
      ...
      data: (interests) {
        ...
        for (final i in interests) { byCategory.putIfAbsent(i.category ?? 'Other', () => []).add(i); }
        return ListView(controller: widget.scrollController, padding: const EdgeInsets.all(16),
          children: byCategory.entries.map((e) => Column(
            ...
            Wrap(spacing: 8, runSpacing: 8, children: e.value.map((i) {
              ...
              return GestureDetector(
                onTap: () => setState(() => sel ? _selectedInterestIds.remove(i.id) : _selectedInterestIds.add(i.id)),
                child: AnimatedContainer(duration: const Duration(milliseconds: 150),
                  ...
                  child: Text(i.trimmed, style: TextStyle(fontFamily: 'Nunito', fontSize: 12,
                    fontWeight: FontWeight.w700, color: sel ? Colors.white : AppColors.textMid))));
            }).toList()),
          ])).toList());
      });
  }
  ...
  Widget _buildTraits(AsyncValue<List<Trait>> traitsAsync) {
    return traitsAsync.when(
      ...
      data: (traits) {
        ...
        for (final t in traits) { byCategory.putIfAbsent(t.category ?? 'Other', () => []).add(t); }
        return ListView(controller: widget.scrollController, padding: const EdgeInsets.all(16),
          children: byCategory.entries.map((e) => Column(
            crossAxisAlignment: CrossAxisAlignment.start, children: [
            Padding(padding: const EdgeInsets.only(top: 12, bottom: 8),
            ...
            ...e.value.map((t) {
              ...
              return Padding(padding: const EdgeInsets.only(bottom: 8),
                child: GestureDetector(
                  onTap: () => setState(() => sel ? _selectedTraitIds.remove(t.id) : _selectedTraitIds.add(t.id)),
                  child: AnimatedContainer(duration: const Duration(milliseconds: 150),
                    ...
                    child: Row(children: [
                      Expanded(child: Text(t.trimmed, style: TextStyle(fontFamily: 'Nunito', fontSize: 13,
                        fontWeight: FontWeight.w600, color: sel ? AppColors.primary : AppColors.textDark))),
...
class _ContactRow extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Padding(
    ...
    child: Row(children: [
      ...
      const SizedBox(width: 12),
      Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Text(label, style: const TextStyle(fontFamily: 'Nunito', fontSize: 13, fontWeight: FontWeight.w700)),
        Text(value, style: const TextStyle(fontFamily: 'Nunito', fontSize: 12, color: AppColors.primary)),
      ])),
    ]));
}

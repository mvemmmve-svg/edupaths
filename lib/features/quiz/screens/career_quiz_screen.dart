...
// Loads from DB. Shows careers grouped by category with search bar.
...
class _QuizQ {
  ...
  const _QuizQ({required this.question, required this.optionA,
    required this.optionB, required this.optionC,
    required this.correctAnswer, required this.fact});
  List<String> get options => [optionA, optionB, optionC];
  int get correctIndex => correctAnswer == 'a' ? 0 : correctAnswer == 'b' ? 1 : 2;
  factory _QuizQ.fromRow(Map<String, dynamic> r) => _QuizQ(
    question: r['question'] as String,
    optionA: r['option_a'] as String, optionB: r['option_b'] as String,
...
class _CareerQuiz {
  ...
  const _CareerQuiz({required this.careerId, required this.careerName,
    required this.category, required this.questions});

  Color get color {
    const m = {
      'Technology': Color(0xFF4F46E5), 'Engineering': Color(0xFF0891B2),
      'Health & Medicine': Color(0xFF059669), 'Business & Finance': Color(0xFF0EA5E9),
      'Creative & Media': Color(0xFFDB2777), 'Law & Social Work': Color(0xFF7C3AED),
  ...
  String get emoji {
    const m = {
      ...
      'doctor': '🏥', 'teacher': '📚', 'accountant': '📊', 'chef': '👨‍🍳',
      'graphic designer': '🎨', 'civil engineer': '🏗️',
      'marketing manager': '📣', 'psychologist': '🧠',
      'lawyer': '⚖️', 'architect': '🏛️', 'data scientist': '📈',
    };
...
class CareerQuizScreen extends StatefulWidget {
  ...
  @override State<CareerQuizScreen> createState() => _CareerQuizScreenState();
}
...
class _CareerQuizScreenState extends State<CareerQuizScreen> {
  ...
  Future<void> _load() async {
    try {
      final rows = await _supabase.from('career_quizzes').select()
          .eq('is_active', true).order('career_name').order('sort_order');
      ...
      // Get career categories from careers table
      final careerIds = grouped.keys.toList();
      final careerRows = await _supabase.from('careers')
          .select('id, category').inFilter('id', careerIds);
      ...
      }).toList()..sort((a, b) => a.careerName.compareTo(b.careerName));

      setState(() { _all = quizzes; _filtered = quizzes; _loading = false; });
    } catch (e) {
    ...
    setState(() {
      _filtered = q.isEmpty ? _all
          : _all.where((c) =>
              ...
              c.category.toLowerCase().contains(lower)).toList();
    });
  }
  ...
  void _answer(int i) {
    ...
    setState(() { _picked = i; if (i == _quiz!.questions[_index].correctIndex) _score++; });
  }

  void _next() => setState(() {
    if (_index < _quiz!.questions.length - 1) { _index++; _picked = null; }
    else { _finished = true; }
  });

  (String, String) get _verdict => switch ((_score * 10 ~/ _quiz!.questions.length)) {
    10 => ('🏆', 'Career Genius! You really know your stuff.'),
    >= 7 => ('🔥', 'So close to perfect — you clearly pay attention.'),
    >= 5 => ('👍', 'Solid! You know more than most people.'),
  ...
  Widget build(BuildContext context) => Scaffold(
    ...
    appBar: AppBar(
      ...
      leading: GestureDetector(
        onTap: () { if (_quiz != null && !_finished) setState(() => _quiz = null); else context.pop(); },
        child: const BackBtn())),
    body: SafeArea(child: _loading
        ...
        : _error != null
            ...
            : _quiz == null ? _buildPicker()
            : _finished ? _buildResults()
            : _buildQuestion()),
  );
  ...
  Widget _buildPicker() {
    // Group by category (already sorted alphabetically)
    ...
    return Column(children: [
      ...
      Expanded(
        child: _filtered.isEmpty
            ? const Center(child: Text('No quizzes match your search',
                      ...
                      child: GestureDetector(
                        ...
                        child: Container(
                          ...
                          decoration: BoxDecoration(
                            ...
                            boxShadow: [BoxShadow(color: q.color.withOpacity(0.3),
                          ...
                          child: Row(children: [
                            ...
                            Expanded(child: Column(
                              ...
                              Text(q.careerName, style: const TextStyle(fontFamily: 'Nunito',
                                  fontSize: 16, fontWeight: FontWeight.w900, color: Colors.white)),
                              Text('${q.questions.length} questions · ~1 min',
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        ClipRRect(borderRadius: BorderRadius.circular(6),
          child: LinearProgressIndicator(
            ...
            minHeight: 8, backgroundColor: AppColors.bgSurface,
            valueColor: AlwaysStoppedAnimation(_quiz!.color))),
        const SizedBox(height: 20),
        Text(q.question, style: const TextStyle(fontFamily: 'Nunito', fontSize: 18,
            fontWeight: FontWeight.w900, color: AppColors.textDark, height: 1.35)),
        const SizedBox(height: 16),
        ...List.generate(q.options.length, (i) {
          Color border = AppColors.border, bg = AppColors.bgCard;
          String? tail;
          if (answered) {
            if (i == q.correctIndex) { border = AppColors.success; bg = const Color(0xFFECFDF5); tail = '✓'; }
            else if (i == _picked) { border = AppColors.error; bg = const Color(0xFFFEF2F2); tail = '✗'; }
          }
          return Padding(
            padding: const EdgeInsets.only(bottom: 10),
            child: GestureDetector(
              ...
              child: AnimatedContainer(
                ...
                child: Row(children: [
                  Expanded(child: Text(q.options[i], style: const TextStyle(fontFamily: 'Nunito',
                      fontSize: 14, fontWeight: FontWeight.w700, height: 1.3))),
                  if (tail != null) Text(tail, style: TextStyle(fontFamily: 'Nunito',
        ...
        if (answered && q.fact.isNotEmpty) ...[
          ...
          Container(
            padding: const EdgeInsets.all(14),
            decoration: BoxDecoration(color: _quiz!.color.withOpacity(0.08),
                borderRadius: BorderRadius.circular(14),
                border: Border.all(color: _quiz!.color.withOpacity(0.3))),
            child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
              const Text('💡 ', style: TextStyle(fontSize: 14)),
              Expanded(child: Text(q.fact, style: const TextStyle(fontFamily: 'Nunito',
                  fontSize: 12.5, color: AppColors.textDark, height: 1.45,
                  fontWeight: FontWeight.w600))),
            ])),
          const SizedBox(height: 16),
          PrimaryBtn(
            label: _index < _quiz!.questions.length - 1 ? 'Next question →' : 'See my score 🏁',
            onPressed: _next),
        ],
      ]));
  }
    ...
    return Center(child: SingleChildScrollView(
      ...
      child: Column(mainAxisSize: MainAxisSize.min, children: [
        ...
        Text('${_quiz!.emoji} ${_quiz!.careerName} quiz complete',
            style: const TextStyle(fontFamily: 'Nunito', fontSize: 12.5, color: AppColors.textMid)),
        const SizedBox(height: 24),
        PrimaryBtn(label: '🧠 Try another career',
            onPressed: () => setState(() => _quiz = null)),
        const SizedBox(height: 6),
        TextButton(onPressed: () => context.go('/home'),
          child: const Text('Back to Home', style: TextStyle(fontFamily: 'Nunito',
              fontSize: 13, fontWeight: FontWeight.w700, color: AppColors.textMid))),
      ])));
  }
}
